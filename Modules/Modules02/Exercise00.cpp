#include <iostream>
#include <string>
#include<memory>  // 메모리 누수를 방지해줌. ( new / delete 사용하지 않아도 됨. )
using namespace std;
class ElectricCar {
private:
	string model;
...
int main() {
	//ElectricCar* A = new ElectricCar("Ioniq 9", 2025, 110);
	//delete A;
	unique_ptr<ElectricCar> A = make_unique<ElectricCar>();
	A->setMod("Ioniq 9");
	A->setYear(2025);
	A->setBett(110);
	A->showCar();

	//SuperCar* B = new SuperCar("Spectre", 2024, 105, 4.5, 250);
	//delete B;
	unique_ptr<SuperCar> B = make_unique<SuperCar>();
	B->setMod("Spectre");
	B->setYear(2024);
	B->setBett(105);
	B->setZtoh(4.5);
